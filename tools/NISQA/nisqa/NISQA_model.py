...
class nisqaModel(object):
    ...
    def _loadDatasets(self):
        ...
        elif self.args['mode']=='predict_pattern':
            self._loadDatasetsPattern()
        else:
    ...
    def _loadDatasetsPattern(self):
        files = glob(self.args['pattern']) 
        dfile = pd.DataFrame(files, columns=["file"])
        dcon = None
        # creating Datasets ---------------------------------------------------                        
        self.ds_val = NL.SpeechQualityDataset(
            dfile,
